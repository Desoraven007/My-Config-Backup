...
class MpvPlayer {
  public:
    ...
    // Vai direto pra um ponto absoluto — usado ao arrastar a barra de
    // progresso (soltar o dedo num ponto específico, não relativo à posição
    // atual como `seek()`).
    ...
    // Tracks expostas pelo mpv (track-list): usadas pelo HUD para escolher
    // áudio/legendas sem depender do OSD interno do mpv.
    std::vector<MpvTrackInfo> getTracks() const;
    void selectAudioTrack(int id);
    void selectSubtitleTrack(int id); // id <= 0 desliga legendas
