...
void MpvPlayer::seekAbsolute(double seconds) {
    ...
    std::string cmd = "seek " + std::to_string(seconds) + " absolute";
    mpv_command_string(handle, cmd.c_str());
}


std::vector<MpvTrackInfo> MpvPlayer::getTracks() const {
    std::vector<MpvTrackInfo> tracks;
    if (!handle) return tracks;

    mpv_node node{};
    if (mpv_get_property(handle, "track-list", MPV_FORMAT_NODE, &node) < 0) return tracks;
    ...
    if (node.format == MPV_FORMAT_NODE_ARRAY && node.u.list) {
        for (int i = 0; i < node.u.list->num; ++i) {
            ...
            for (int j = 0; j < entry.u.list->num; ++j) {
                ...
                } else if (strcmp(key, "type") == 0 && value.format == MPV_FORMAT_STRING && value.u.string) {
                    track.type = value.u.string;
                } else if (strcmp(key, "lang") == 0 && value.format == MPV_FORMAT_STRING && value.u.string) {
                    track.language = value.u.string;
                } else if (strcmp(key, "title") == 0 && value.format == MPV_FORMAT_STRING && value.u.string) {
                    track.title = value.u.string;
                } else if (strcmp(key, "selected") == 0 && value.format == MPV_FORMAT_FLAG) {
                    track.selected = value.u.flag != 0;
                }
            }
            if (track.id > 0 && (track.type == "audio" || track.type == "sub")) tracks.push_back(std::move(track));
        }
    }
...
void MpvPlayer::selectAudioTrack(int id) {
    ...
    mpv_set_property_string(handle, "aid", std::to_string(id).c_str());
}

void MpvPlayer::selectSubtitleTrack(int id) {
    if (!handle) return;
    if (id <= 0) mpv_set_property_string(handle, "sid", "no");
    else mpv_set_property_string(handle, "sid", std::to_string(id).c_str());
}

bool MpvPlayer::isBuffering() const {
    if (!handle) return false;
    int flag = 0;
    if (mpv_get_property(handle, "paused-for-cache", MPV_FORMAT_FLAG, &flag) < 0) return false;
    return flag != 0;
}
